...
void check_batch(std::vector<float> const &tensor, int batch_size) {
    ...
    for (size_t j = 0; j < batch_size; j++) {
        ...
        if (j == 0)
            last_accum = accum;
        else if (accum != last_accum) { throw std::runtime_error(""); }
    }
}
...
memory do_conv(engine &eng, stream &s, std::vector<primitive> &net,
        ...
        memory &src_memory, memory::desc &dst_md, bool with_bias = true) {
    ...
    if (with_bias) bias_md = memory::desc {{bias_dims}, dt::f32, tag::any};
...
memory do_conv(engine &eng, stream &s, std::vector<primitive> &net,
        std::vector<std::unordered_map<int, memory>> &net_args,
        memory &src_usr_memory, void *weights_data, void *bias_data,
        memory::dims &weights_dims, memory::dims &bias_dims,
        memory::dims &strides, memory::dims &padding, memory::dims &src_dims,
        memory::desc &dst_md, bool with_bias = true) {
    ...
    if (prim_desc.src_desc() != src_usr_memory.get_desc()) {
        ...
        reorder::primitive_desc r_pd(eng, src_usr_memory.get_desc(), eng,
                prim_desc.src_desc(), primitive_attr());
    ...
    if (prim_desc.weights_desc() != weights_user_memory.get_desc()) {
        reorder::primitive_desc r_pd(eng, weights_md_nhwc, eng,
                prim_desc.weights_desc(), primitive_attr());
...
class res_net_block {
public:
    res_net_block(engine &eng, stream &s, std::vector<primitive> &net,
            std::vector<std::unordered_map<int, memory>> &net_args,
        ...
        auto conv_dst_md = memory::desc(dst_dims_, dt::f32, tag::any);
        auto bnorm_dst_md = memory::desc(dst_dims_, dt::f32, tag::nhwc);
        dst_mem_ = memory(bnorm_dst_md, eng);
        ...
        dst_mem_ = do_conv(eng, s, net, net_args, weights_data_.data(),
                params.with_bias ? bias_data_.data() : nullptr, weights_dims_,
                bias_dims_, params.strides, params.padding, src_mem_,
                conv_dst_md, false);
...
void res_net(engine::kind engine_kind, int times = 100,
    ...
    auto conv1_src_md = memory::desc(conv1_src_dims, dt::f32, tag::nhwc);
    auto conv1_src_memory = memory(conv1_src_md, eng);
    write_to_dnnl_memory(src_data_copied.data(), conv1_src_memory);
    auto conv1_dst_md = memory::desc(conv1_dst_dims, dt::f32, tag::any);
    ...
    auto conv1_dst_memory = do_conv(eng, s, net, net_args, conv1_src_memory,
            weights_1.data(), biases_1.data(), conv1_weights_dims,
            conv1_bias_dims, conv1_strides, conv1_padding, conv1_src_dims,
            conv1_dst_md, false);
    ...
    res_net_block block_2a1(
            eng, s, net, net_args, pool1_dst_memory, block_2a1_params);
    ...
    res_net_block block_2b2a(
            eng, s, net, net_args, res2a_add_dst_memory, block_2b2a_params);

    block_params block_2b2b_params {2, 2, 2, 2, true, false, BATCH, 64, 64, 56,
            56, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_2b2b(
            eng, s, net, net_args, block_2b2a.get_dst_mem(), block_2b2b_params);

    block_params block_2b2c_params {2, 2, 2, 3, false, false, BATCH, 64, 256,
            56, 56, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_2b2c(
            eng, s, net, net_args, block_2b2b.get_dst_mem(), block_2b2c_params);
    ...
    res_net_block block_2c2a(
            eng, s, net, net_args, res2b_add_dst_memory, block_2c2a_params);

    block_params block_2c2b_params {2, 3, 2, 2, true, false, BATCH, 64, 64, 56,
            56, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_2c2b(
            eng, s, net, net_args, block_2c2a.get_dst_mem(), block_2c2b_params);

    block_params block_2c2c_params {2, 3, 2, 3, false, false, BATCH, 64, 256,
            56, 56, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_2c2c(
            eng, s, net, net_args, block_2c2b.get_dst_mem(), block_2c2c_params);
    ...
    res_net_block block_3a1(
            eng, s, net, net_args, res2c_add_dst_memory, block_3a1_params);
    ...
    res_net_block block_3a2a(
            eng, s, net, net_args, res2c_add_dst_memory, block_3a2a_params);

    /* -----------------conv > bnorm > scale > relu----------------- */
    block_params block_3a2b_params {3, 1, 2, 2, true, false, BATCH, 128, 128,
            28, 28, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_3a2b(
            eng, s, net, net_args, block_3a2a.get_dst_mem(), block_3a2b_params);

    /* -----------------conv > bnorm > scale----------------- */
    block_params block_3a2c_params {3, 1, 2, 3, false, false, BATCH, 128, 512,
            28, 28, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_3a2c(
            eng, s, net, net_args, block_3a2b.get_dst_mem(), block_3a2c_params);
    ...
    res_net_block block_3c2a(
            eng, s, net, net_args, res3b_add_dst_memory, block_3c2a_params);

    block_params block_3c2b_params {3, 3, 2, 2, true, false, BATCH, 128, 128,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_3c2b(
            eng, s, net, net_args, block_3c2a.get_dst_mem(), block_3c2b_params);

    block_params block_3c2c_params {3, 3, 2, 3, false, false, BATCH, 128, 512,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_3c2c(
            eng, s, net, net_args, block_3c2b.get_dst_mem(), block_3c2c_params);
    ...
    res_net_block block_3d2a(
            eng, s, net, net_args, res3c_add_dst_memory, block_3d2a_params);

    block_params block_3d2b_params {3, 4, 2, 2, true, false, BATCH, 128, 128,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_3d2b(
            eng, s, net, net_args, block_3d2a.get_dst_mem(), block_3d2b_params);

    block_params block_3d2c_params {3, 4, 2, 3, false, false, BATCH, 128, 512,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_3d2c(
            eng, s, net, net_args, block_3d2b.get_dst_mem(), block_3d2c_params);
    ...
    res_net_block block_4a1(
            eng, s, net, net_args, res3d_add_dst_memory, block_4a1_params);
    ...
    res_net_block block_4a2a(
            eng, s, net, net_args, res3d_add_dst_memory, block_4a2a_params);
    ...
    res_net_block block_4a2b(
            eng, s, net, net_args, block_4a2a.get_dst_mem(), block_4a2b_params);
    ...
    res_net_block block_4a2c(
            eng, s, net, net_args, block_4a2b.get_dst_mem(), block_4a2c_params);
    ...
    res_net_block block_4b2a(
            eng, s, net, net_args, res4a_add_dst_memory, block_4b2a_params);
    IC = 256, OC = 256;
    block_params block_4b2b_params {4, 2, 2, 2, true, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_4b2b(
            eng, s, net, net_args, block_4b2a.get_dst_mem(), block_4b2b_params);
    IC = 256, OC = 1024;
    block_params block_4b2c_params {4, 2, 2, 3, false, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_4b2c(
            eng, s, net, net_args, block_4b2b.get_dst_mem(), block_4b2c_params);
    ...
    res_net_block block_4c2a(
            eng, s, net, net_args, res4b_add_dst_memory, block_4c2a_params);
    IC = 256, OC = 256;
    block_params block_4c2b_params {4, 3, 2, 2, true, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_4c2b(
            eng, s, net, net_args, block_4c2a.get_dst_mem(), block_4c2b_params);
    IC = 256, OC = 1024;
    block_params block_4c2c_params {4, 3, 2, 3, false, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_4c2c(
            eng, s, net, net_args, block_4c2b.get_dst_mem(), block_4c2c_params);
    ...
    res_net_block block_4d2a(
            eng, s, net, net_args, res4c_add_dst_memory, block_4d2a_params);
    IC = 256, OC = 256;
    block_params block_4d2b_params {4, 4, 2, 2, true, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_4d2b(
            eng, s, net, net_args, block_4d2a.get_dst_mem(), block_4d2b_params);
    IC = 256, OC = 1024;
    block_params block_4d2c_params {4, 4, 2, 3, false, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_4d2c(
            eng, s, net, net_args, block_4d2b.get_dst_mem(), block_4d2c_params);
    ...
    res_net_block block_4e2a(
            eng, s, net, net_args, res4d_add_dst_memory, block_4e2a_params);
    IC = 256, OC = 256;
    block_params block_4e2b_params {4, 5, 2, 2, true, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_4e2b(
            eng, s, net, net_args, block_4e2a.get_dst_mem(), block_4e2b_params);
    IC = 256, OC = 1024;
    block_params block_4e2c_params {4, 5, 2, 3, false, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_4e2c(
            eng, s, net, net_args, block_4e2b.get_dst_mem(), block_4e2c_params);
    ...
    res_net_block block_4f2a(
            eng, s, net, net_args, res4e_add_dst_memory, block_4f2a_params);
    IC = 256, OC = 256;
    block_params block_4f2b_params {4, 6, 2, 2, true, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_4f2b(
            eng, s, net, net_args, block_4f2a.get_dst_mem(), block_4f2b_params);
    IC = 256, OC = 1024;
    block_params block_4f2c_params {4, 6, 2, 3, false, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_4f2c(
            eng, s, net, net_args, block_4f2b.get_dst_mem(), block_4f2c_params);
    ...
    res_net_block block_5a1(
            eng, s, net, net_args, res4f_add_dst_memory, block_5a1_params);
    ...
    res_net_block block_5a2a(
            eng, s, net, net_args, res4f_add_dst_memory, block_5a2a_params);
    ...
    res_net_block block_5a2b(
            eng, s, net, net_args, block_5a2a.get_dst_mem(), block_5a2b_params);
    ...
    res_net_block block_5a2c(
            eng, s, net, net_args, block_5a2b.get_dst_mem(), block_5a2c_params);
    ...
    res_net_block block_5b2a(
            eng, s, net, net_args, res5a_add_dst_memory, block_5b2a_params);
    IC = 512, OC = 512;
    block_params block_5b2b_params {5, 2, 2, 2, true, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_5b2b(
            eng, s, net, net_args, block_5b2a.get_dst_mem(), block_5b2b_params);
    IC = 512, OC = 2048;
    block_params block_5b2c_params {5, 2, 2, 3, false, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_5b2c(
            eng, s, net, net_args, block_5b2b.get_dst_mem(), block_5b2c_params);
    ...
    res_net_block block_5c2a(
            eng, s, net, net_args, res5b_add_dst_memory, block_5c2a_params);
    IC = 512, OC = 512;
    block_params block_5c2b_params {5, 3, 2, 2, true, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 3, 3, {1, 1}, {1, 1}};
    res_net_block block_5c2b(
            eng, s, net, net_args, block_5c2a.get_dst_mem(), block_5c2b_params);
    IC = 512, OC = 2048;
    block_params block_5c2c_params {5, 3, 2, 3, false, false, BATCH, IC, OC,
            WIDTH, HEIGHT, 1, 1, {0, 0}, {1, 1}};
    res_net_block block_5c2c(
            eng, s, net, net_args, block_5c2b.get_dst_mem(), block_5c2c_params);
